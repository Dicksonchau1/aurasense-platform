...
export default function LoginPage() {
  ...
  const tagL = "label";
  const LabelTag = tagL as keyof JSX.IntrinsicElements;

  return (
    <main style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui", background: "#0a0e15", color: "#e5e7eb" }}>
      <div style={cardStyle}>
        <h1 style={{ fontSize: 24, fontWeight: 700, marginTop: 0, marginBottom: 4 }}>ATLAS</h1>
        <p style={{ color: "#9ca3af", marginTop: 0, marginBottom: 24, fontSize: 14 }}>Sign in to your AuraSense console</p>

        {error && (
          <div style={{ background: "#7f1d1d", color: "#fecaca", padding: 10, borderRadius: 6, marginBottom: 16, fontSize: 13 }}>
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleCredentials}>
          ...
          <input type="password" required value={password} onChange={e => setPassword(e.target.value)} style={{ ...inputStyle, marginBottom: 16 }} />

          <button type="submit" disabled={busy}
            style={{ width: "100%", padding: "10px 14px", background: "#22d3ee", color: "#0a0e15", border: "none", borderRadius: 6, fontSize: 15, fontWeight: 600, cursor: busy ? "not-allowed" : "pointer", opacity: busy ? 0.6 : 1 }}>
