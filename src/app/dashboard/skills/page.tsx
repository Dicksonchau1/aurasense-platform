...
export default function SkillsPage() {
  ...
  const [schemaTab, setSchemaTab] = useState<"schema"|"code"|"deploy">("schema");
      ...
      {selected && (
        <div style={{position:"fixed",inset:0,zIndex:1000,display:"flex",justifyContent:"flex-end"}} onClick={()=>setSelected(null)}>
          <div style={{width:420,height:"100%",background:"#0a1628",border:"1px solid #1a1f26",borderRight:"none",overflowY:"auto",padding:20}} onClick={e=>e.stopPropagation()}>
            ...
            {/* Agent Schema & Code Writer */}
            <div style={{marginBottom:14,borderTop:"1px solid #1a1f26",paddingTop:14}}>
              <div style={{fontSize:11,fontWeight:700,color:"#5ab8d0",textTransform:"uppercase",letterSpacing:".06em",marginBottom:8}}>Agent Schema &amp; Code</div>
              <div style={{display:"flex",gap:4,marginBottom:10}}>
                {(["schema","code","deploy"] as const).map(t=>(
                  <button key={t} onClick={()=>setSchemaTab(t)} style={{padding:"4px 10px",borderRadius:6,fontSize:10.5,fontWeight:600,cursor:"pointer",border:"none",background:schemaTab===t?"rgba(79,152,163,.3)":"rgba(255,255,255,.05)",color:schemaTab===t?"#5ab8d0":"#8b9aae"}}>
                    {t==="schema"?"JSON Schema":t==="code"?"Agent Code":"Deploy to Map"}
                  </button>
                ))}
              </div>
              {schemaTab==="schema" && (
                <pre style={{background:"rgba(0,0,0,.4)",border:"1px solid #1a1f26",borderRadius:8,padding:12,fontSize:10,color:"#a8d5dc",fontFamily:"ui-monospace,monospace",overflowX:"auto",maxHeight:200,overflowY:"auto",margin:0,whiteSpace:"pre-wrap"}}>{JSON.stringify({
                  "$schema":"https://nepa.aurasense.io/schemas/skill/v1",
                  skillId:selected.id, name:selected.name, version:selected.version,
                  category:selected.category, status:selected.status,
                  description:selected.description, author:selected.author,
                  parameters:Object.fromEntries(Object.entries(selected.params).map(([k,v])=>[k,{type:"string",default:v,description:`Parameter: ${k}`}])),
                  deployment:{targets:selected.deployedTo,sla:"standard",region:"HK"},
                  nepa:{pipeline_stage:"skill_execution",agent_loop:true,world_model_binding:true},
                },null,2)}</pre>
              )}
              {schemaTab==="code" && (
                <pre style={{background:"rgba(0,0,0,.4)",border:"1px solid #1a1f26",borderRadius:8,padding:12,fontSize:10,color:"#a8d5dc",fontFamily:"ui-monospace,monospace",overflowX:"auto",maxHeight:200,overflowY:"auto",margin:0,whiteSpace:"pre-wrap"}}>{`# NEPA Agent — Auto-generated by AuraSense Atlas\n# Skill: ${selected.name} (${selected.id})\n\nfrom nepa_sdk import SkillAgent, WorldModel, DroneFleet\n\nclass ${selected.name.replace(/[^a-zA-Z0-9]/g,"")}Agent(SkillAgent):\n    skill_id = "${selected.id}"\n    version  = "${selected.version}"\n    category = "${selected.category}"\n\n    def __init__(self):\n        super().__init__()\n${Object.entries(selected.params).map(([k,v])=>`        self.${k} = "${v}"`).join("\n")}\n\n    async def perceive(self, world: WorldModel):\n        self.scene = await world.get_scene()\n        self.drones = await DroneFleet.get_active()\n        return self.scene\n\n    async def plan(self, scene):\n        return await self.nepa.plan(scene, skill_id=self.skill_id)\n\n    async def act(self, plan):\n        for drone in self.drones:\n            await drone.execute(plan)\n        await self.audit_log(plan)\n\n    async def run(self):\n        while True:\n            scene = await self.perceive(WorldModel())\n            plan  = await self.plan(scene)\n            await self.act(plan)\n            await self.sleep(1.0 / 30)  # 30 Hz`}</pre>
              )}
              {schemaTab==="deploy" && (
                <div>
                  <div style={{fontSize:11,color:"#8b9aae",marginBottom:10}}>Deploy this skill as an active zone on the World Model map</div>
                  <div style={{marginBottom:10}}>
                    <label style={{fontSize:11,color:"#8b9aae",display:"block",marginBottom:4}}>Target Building</label>
                    <select value={wmBuilding} onChange={e=>setWmBuilding(e.target.value)}
                      style={{width:"100%",height:30,padding:"0 9px",background:"rgba(255,255,255,.06)",border:"1px solid #1a1f26",borderRadius:6,color:"#cfd8e3",fontSize:12,outline:"none"}}>
                      {WM_BUILDINGS.map(b=><option key={b} value={b} style={{background:"#0a1628"}}>{b}</option>)}
                    </select>
                  </div>
                  <button onClick={()=>{ deploySkill(selected); showToast(`${selected.name} deployed to World Model → ${wmBuilding}`); }} style={{...btnT,width:"100%",justifyContent:"center",marginBottom:6}}>⚡ Deploy to World Model</button>
                  <button onClick={()=>{
                    const schema=JSON.stringify({skillId:selected.id,name:selected.name,version:selected.version,parameters:selected.params,deployment:{target:wmBuilding,region:"HK",sla:"standard"}},null,2);
                    const blob=new Blob([schema],{type:"application/json"});
                    const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download=`${selected.id}-schema.json`; a.click();
                    showToast("Schema downloaded");
                  }} style={{...btnG,width:"100%",justifyContent:"center"}}>↓ Download Schema JSON</button>
                </div>
              )}
            </div>
